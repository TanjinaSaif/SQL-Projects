...
WHERE 
   ...
   SELECT first_name,last_name , salary FROM employees ORDER BY salary DESC LIMIT 0 , 5;
...
SELECT first_name,last_name
FROM employees
WHERE department_id  IN (SELECT department_id FROM departments WHERE department_name = 'IT' OR department_name = 'Administration');
...
WHERE department_id  IN (SELECT department_id FROM departments
 ...
 SELECT salary FROM employees ORDER BY salary LIMIT 4, 1;
...
Where employee_id In (Select manager_id from departments where location_id IN 
